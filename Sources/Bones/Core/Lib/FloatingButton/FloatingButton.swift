...
private struct FloatingButtonKey: EnvironmentKey {
  static let defaultValue: Binding<Bool> = .constant(false)
}

extension EnvironmentValues {
  var floatingButtonIsOpen: Binding<Bool> {
    get { self[FloatingButtonKey.self] }
    set { self[FloatingButtonKey.self] = newValue }
  }
}
...
public struct FloatingButtonContent {
...
public struct FloatingButton: View {
  ...
  fileprivate var mainButton: LocalizedStringKey
  ...
  fileprivate var animation: Animation = .spring(duration: 0.5, bounce: 0.5, blendDuration: 0.8)
  ...
  // straight
  fileprivate var direction: Direction = .top
  ...
  public init(_ title: LocalizedStringKey, buttons: [FloatingButtonContent], isMainButtonLeading: Bool = false) {
    self.mainButton = title
    self.isMainButtonLeading = isMainButtonLeading
    self.buttons = []

    initButtons(buttons)
  }

  mutating func initButtons(_ buttons: [FloatingButtonContent]) {
    ...
    ZStack(alignment: mainZStackAlignment) {
      ForEach((0..<buttons.count), id: \.self) { i in
        buttons[i]
          ...
          .onTapGesture {
            privateIsOpen.toggle()
            buttons[i].action()
          }
      }

      MainButtonViewInternal(isOpen: isOpenBinding ?? $privateIsOpen, isLeading: isMainButtonLeading, buttonTitle: mainButton)
//        .buttonStyle(BonesNavigationButton())
    ...
    .onChange(of: mainButtonSize) {
      calculateCoords()
    }
    .environment(\.floatingButtonIsOpen, $privateIsOpen)
  }
...
struct SubmenuButton: View {
  @Environment(\.floatingButtonIsOpen) @Binding var isOpen: Bool
...
private struct MainButtonViewInternal: View {
  ...
  var isLeading: Bool
  fileprivate var buttonTitle: LocalizedStringKey

  public var body: some View {
    Button(buttonTitle) { isOpen.toggle() }
